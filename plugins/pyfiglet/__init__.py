...
class FigletFont(object):
    ...
    def preloadFont(cls, font):
        ...
        for extension in ('tlf', 'flf'):
            ...
            if pkg_resources.resource_exists('plugins.pyfiglet.fonts', fn):
                return pkg_resources.resource_string('plugins.pyfiglet.fonts', fn)
        else:
    ...
    def getFonts(cls):
        return [font.rsplit('.', 2)[0] for font
                in pkg_resources.resource_listdir('plugins.pyfiglet', 'fonts')
                if font.endswith(('.flf', '.tlf'))
                   and cls.reMagicNumber.search(pkg_resources.resource_stream(
                        'plugins.pyfiglet.fonts', font).readline())]
